"""
This Pytorch TiDE implementation is taken from TSLib:
https://github.com/thuml/Time-Series-Library
The original implementation with TensorFlow can be found here:
...
class LayerNorm(nn.Module):
    ...
    def forward(self, input):
        return func.layer_norm(input, self.weight.shape, self.weight,
                               self.bias,
                               1e-5)
...
class Tide(nn.Module):
    ...
    def __init__(self, params, bias=True, feature_encode_dim=2):
        super().__init__()
        self.params = params
        self.task_name = params.task_name
        self.seq_len = params.seq_len  # L
        self.label_len = params.data_overlap
        self.pred_len = params.pred_len  # H
        self.hidden_dim = params.model_dim
        self.res_hidden = params.model_dim
        self.encoder_num = params.num_encoder_layers
        self.decoder_num = params.num_decoder_layers
        self.freq = params.freq
        self.feature_encode_dim = feature_encode_dim
        self.decode_dim = params.c_tide_out + 1
        self.temporalDecoderHidden = params.ff_dim
        ...
        self.encoders = nn.Sequential(
            ResBlock(flatten_dim, self.res_hidden, self.hidden_dim, dropout,
                     bias), *([ResBlock(self.hidden_dim, self.res_hidden,
                                        self.hidden_dim, dropout, bias)] * (
                                      self.encoder_num - 1)))
        if (self.task_name == 'long_term_forecast' or self.task_name ==
                'short_term_forecast'):
            self.decoders = nn.Sequential(*([ResBlock(self.hidden_dim,
                                                      self.res_hidden,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, batch_y_mark):
        ...
        dec_out = self.temporalDecoder(
            torch.cat([feature[:, self.seq_len:], decoded], dim=-1)).squeeze(
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, batch_y_mark, mask=None):
        """x_mark_enc is the exogenous dynamic feature described
        ...
        if (self.task_name == 'long_term_forecast' or self.task_name ==
            ...
            dec_out = torch.stack([self.forecast(x_enc[:, :, feature],
                                                 x_mark_enc, x_dec,
                                                 batch_y_mark) for feature in
                                   range(x_enc.shape[-1])], dim=-1)
            return dec_out  # [B, L, D]
        if self.task_name == 'imputation':
            dec_out = torch.stack([self.imputation(x_enc[:, :, feature],
                                                   x_mark_enc, x_dec,
                                                   batch_y_mark, mask) for
                                   feature in range(x_enc.shape[-1])], dim=-1)
            return dec_out  # [B, L, D]
        return None
