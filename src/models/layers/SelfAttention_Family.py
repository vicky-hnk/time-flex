...
from src.util.masking import TriangularCausalMask, ProbMask
...
class DSAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask, tau=None, delta=None):
...
class ProbAttention(nn.Module):
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attention_mask):
        ...
        torch.arange(H)[None, :, None], index, :] = torch.matmul(attn,
                                                                 V).type_as(
            context_in)
        if self.output_attention:
...
class AttentionLayer(nn.Module):
    def __init__(self, attention, model_dim, num_heads, d_keys=None,
                 d_values=None):
        super(AttentionLayer, self).__init__()

        d_keys = d_keys or (model_dim // num_heads)
        d_values = d_values or (model_dim // num_heads)

        self.inner_attention = attention
        self.query_projection = nn.Linear(model_dim, d_keys * num_heads)
        self.key_projection = nn.Linear(model_dim, d_keys * num_heads)
        self.value_projection = nn.Linear(model_dim, d_values * num_heads)
        self.out_projection = nn.Linear(d_values * num_heads, model_dim)
...
class ReformerLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask, tau, delta):
...
class TwoStageAttentionLayer(nn.Module):
    ...
    input/output shape: [batch_size, Data_dim(D), Seg_num(L), model_dim]
    """

    def __init__(self, params, seg_num, factor, model_dim, num_heads,
                 d_ff=None, dropout=0.1):
        super(TwoStageAttentionLayer, self).__init__()
        d_ff = d_ff or 4 * model_dim
        ...
        self.MLP1 = nn.Sequential(nn.Linear(model_dim, d_ff),
                                  nn.GELU(),
                                  nn.Linear(d_ff, model_dim))
        self.MLP2 = nn.Sequential(nn.Linear(model_dim, d_ff),
                                  nn.GELU(),
                                  nn.Linear(d_ff, model_dim))
    ...
    def forward(self, x, attention_mask=None, tau=None, delta=None):
        ...
        time_in = rearrange(
            x,
            'b ts_d seg_num model_dim -> (b ts_d) seg_num model_dim')
        time_enc, attn = self.time_attention(
            time_in, time_in, time_in, attention_mask=None, tau=None, delta=None
        )
        ...
        dim_send = rearrange(
            dim_in,
            '(b ts_d) seg_num model_dim -> (b seg_num) ts_d model_dim',
            b=batch)
        batch_router = repeat(
            self.router,
        ...
        dim_buffer, attn = self.dim_sender(
            batch_router, dim_send, dim_send, attention_mask=None, tau=None,
            delta=None)
        dim_receive, attn = self.dim_receiver(dim_send, dim_buffer, dim_buffer,
                                              attention_mask=None, tau=None,
                                              delta=None)
        ...
        final_out = rearrange(
            dim_enc, '(b seg_num) ts_d model_dim -> b ts_d seg_num model_dim',
            b=batch)
...
class VarCorAttention(nn.Module):
    def __init__(self, args, mask_flag=True, factor=5, scale=None,
                 attention_dropout=0.1, output_attention=False) -> None:
    ...
    def forward(self, queries, keys, values, attention_mask, tau=None, delta=None):
        ...
        if self.mask_flag:
            if attention_mask is None:
                attention_mask = TriangularCausalMask(B, T, device=queries.device)
