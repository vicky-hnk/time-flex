    ...
    if base == 'legendre':
        ...
        for ki in range(k):
            if ki == 0:
                ...
                coeff_ = Poly(
                    chebyshevt(ki, 2 * x - 1), x).all_coeffs()
                phi_coeff[ki, :ki + 1] = np.flip(
                    2 / np.sqrt(np.pi) * np.array(coeff_).astype(np.float64))
                coeff_ = Poly(
                    chebyshevt(ki, 4 * x - 1), x).all_coeffs()
                phi_2x_coeff[ki, :ki + 1] = np.flip(
                    np.sqrt(2) * 2 / np.sqrt(np.pi) *
                    np.array(coeff_).astype(np.float64))
        ...
        # x_m[x_m==0.5] = 0.5 + 1e-8 # add small noise to avoid the case
        # of 0.5 belonging to both phi(2x) and phi(2x-1)
        ...
        for ki in range(k):
            ...
            for i in range(k):
                proj_ = (
                        wm * phi[i](x_m) * np.sqrt(2) * phi[ki](2 * x_m)).sum()
                psi1_coeff[ki, :] -= proj_ * phi_coeff[i, :]
                psi2_coeff[ki, :] -= proj_ * phi_coeff[i, :]

            for j in range(ki):
                proj_ = (
                        wm * psi1[j](x_m) * np.sqrt(2) * phi[ki](
                    2 * x_m)).sum()
                psi1_coeff[ki, :] -= proj_ * psi1_coeff[j, :]
                psi2_coeff[ki, :] -= proj_ * psi2_coeff[j, :]
...
def get_filter(base, k):
    ...
    if base == 'legendre':
        ...
        x_m = np.array([rt.evalf(20) for rt in roots]).astype(np.float64)
        wm = (1 / k / legendreDer(k, 2 * x_m - 1) /
              eval_legendre(k - 1, 2 * x_m - 1))

        for ki in range(k):
            for kpi in range(k):
                H0[ki, kpi] = 1 / np.sqrt(2) * (wm * phi[ki](x_m / 2) *
                                                phi[kpi](x_m)).sum()
                G0[ki, kpi] = (1 / np.sqrt(2) *
                               (wm * psi(psi1, psi2, ki, x_m / 2) *
                                phi[kpi](x_m)).sum())
                H1[ki, kpi] = (1 / np.sqrt(2) *
                               (wm * phi[ki]((x_m + 1) / 2) *
                                phi[kpi](x_m)).sum())
                G1[ki, kpi] = (1 / np.sqrt(2) *
                               (wm * psi(psi1, psi2, ki, (x_m + 1) / 2) *
                                phi[kpi](x_m)).sum())
...
class MultiWaveletTransform(nn.Module):
    ...
    def __init__(self, ich=1, k=8, alpha=16, c=128,
        ...
        self.ich = ich
        self.MWT_CZ = nn.ModuleList(MWT_CZ1d(k, alpha, L, c, base)
                                    for i in range(nCZ))
...
class FourierCrossAttentionW(nn.Module):
    ...
    def compl_mul1d(self, order, x, weights):
        ...
        if not torch.is_complex(weights):
            w_flag = False
            weights = torch.complex(weights, torch.zeros_like(weights).to(
                weights.device))
        if x_flag or w_flag:
            return torch.complex(
                torch.einsum(
                    order, x.real, weights.real) -
                torch.einsum(order,
                             x.imag,
                             weights.imag),
                torch.einsum(
                    order, x.real, weights.imag)
                + torch.einsum(order,
                               x.imag,
                               weights.real))
        else:
    ...
    def forward(self, q, k, v, mask):
        ...
        xq = q.permute(0, 3, 2,
                       1)  # size = [B, H, E, L] torch.Size([3, 8, 64, 512])
        ...
        xk_ft_ = torch.zeros(B, H, E, len(self.index_k_v), device=xq.device,
                             dtype=torch.cfloat)
        ...
        xqkvw = xqkv_ft
        out_ft = torch.zeros(B, H, E, L // 2 + 1, device=xq.device,
                             dtype=torch.cfloat)
        for i, j in enumerate(self.index_q):
            out_ft[:, :, :, j] = xqkvw[:, :, :, i]
...
class sparseKernelFT1d(nn.Module):
    def __init__(self,
        ...
        self.weights1 = nn.Parameter(
            self.scale * torch.rand(c * k, c * k, self.modes1,
                                    dtype=torch.float))
    ...
    def forward(self, x):
        ...
        out_ft = torch.zeros(B, c * k, N // 2 + 1, device=x.device,
                             dtype=torch.cfloat)
        out_ft[:, :, :l] = self.compl_mul1d("bix,iox->box", x_fft[:, :, :l],
                                            torch.complex(self.weights1,
                                                          self.weights2)[:, :,
                                            :l])
