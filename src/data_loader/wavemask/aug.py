...
#  Chen, M., Xu, Z., Zeng, A., & Xu, Q. (2023). "FrAug: Frequency Domain
#  Augmentation for Time Series Forecasting".
#  arXiv preprint arXiv:2302.09292.
#
# The emd_aug and mix_aug for STAug method are adapted from the following
# source:
...
class augmentation:
    ...
    @staticmethod
    def freq_mask(x: torch.Tensor, y: torch.Tensor, rate: float = 0.5,
        ...
        """
            ...
            - dim (int): Dimension along to concatenate and apply Fourier
            Transform.
        ...
        m = torch.FloatTensor(xy_f.shape).uniform_().to(xy_f.device) < rate
    ...
    @staticmethod
    def freq_mix(x: torch.Tensor, y: torch.Tensor, rate: float = 0.5,
                 dim: int = 1) -> torch.Tensor:
        """
    ...
    @staticmethod
    def wave_mask(x: torch.Tensor, y: torch.Tensor, rates: List[float],
        ...
        """
            ...
            - x (torch.Tensor): Look-back window.
            - y (torch.Tensor): Target horizon.
            - rates (list of floats): List of mask rates for each wavelet
            level.
        ...
        rate_tensor = torch.tensor(rates,
                                   device=x.device)  # Convert rates to tensor

        # Permute dimensions to match expected input:
        # (batch_size, num_features, seq_len)
        xy = xy.permute(0, 2, 1).to(x.device)

        # Initialize & perform the DWT
        dwt = DWT1DForward(J=level, wave=wavelet, mode='symmetric').to(
            x.device)
        cA, cDs = dwt(xy)
        ...
        for i, cD in enumerate(cDs, 1):
            mask_cD = torch.rand_like(cD).to(cD.device) < rate_tensor[
                i]  # Create mask
    ...
    @staticmethod
    def wave_mix(x: torch.Tensor, y: torch.Tensor, rates: List[float],
                 wavelet: str = 'db1', level: int = 2,
                 dim: int = 1) -> torch.Tensor:
        """
        ...
        mask = torch.rand_like(cA1).to(cA1.device) < rate_tensor[
            0]  # Create mask
        ...
        for i, (cD1, cD2) in enumerate(zip(cDs1, cDs2), 1):
            mask = torch.rand_like(cD1).to(cD1.device) < rate_tensor[
                i]  # Create mask
    ...
    def mix_aug(self, batch_x: np.ndarray, batch_y: np.ndarray,
                lambd: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
        """
